🌎 InfoTrack — Sistema de Busca de Notícias

O InfoTrack é uma aplicação web desenvolvida em Django que permite ao usuário pesquisar e filtrar notícias em tempo real, integrando-se com uma API externa de notícias.
O sistema oferece uma interface moderna, intuitiva e responsiva, permitindo a busca por tópicos, categorias e datas, tornando mais fácil acompanhar o que acontece no mundo de forma prática e centralizada.


📰 Principais Funcionalidades:

🔍 Busca inteligente de notícias por palavra-chave
🗓️ Filtragem por data e categoria
🌐 Integração com API externa (NewsAPI)
📱 Design responsivo e intuitivo
⚡ Paginação dinâmica e carregamento otimizado


🧩 Tecnologias Utilizadas:

Python 3 / Django 5
HTML5 / CSS3 / JavaScript
Bootstrap 5
API externa (NewsAPI.org)
Padrão MVC (Model-View-Controller)


👩‍💻 Equipe de Desenvolvimento – UNIMAR

Júlia Lombardi	(14) 99806-4771
João Pedro	(14) 98173-3340
Nathan Gabriel	(14) 99878-7236
Ivan Luiz	(14) 99609-6549
Lucas de Azevedo	(14) 98825-6065
Tainá Alves	(14) 99826-0594
Samuel Alves	(14) 99853-5057
Cawe	(14) 92000-9023

💡 Objetivo do Projeto

O InfoTrack foi desenvolvido como parte de um projeto acadêmico da UNIMAR, com o objetivo de aplicar os conhecimentos de desenvolvimento web, consumo de APIs REST e estruturação de sistemas Django.
O sistema visa demonstrar o potencial de integração entre dados públicos e interfaces modernas, incentivando a curiosidade e a pesquisa tecnológica dos alunos.
